...
modello= LpProblem(sense=LpMinimize)
...
#Stampo i risultati
print(f"objective: {modello.objective.value()}","è il costo minimo")
print(f"x: {x.value()}")
print(f"y: {y.value()}")
...
if b3==0:
    plt.axvline(x=c3,color='darkgreen')
if b4==0:
...
ax.scatter(x.value(), y.value(), s=50, color='red',label='punto di ottimo')
